...
export default class Section {
  ...
  static list() {
    return Promise.resolve(Section.sections);
  }

  static update() {
    Promise.map(Section.sections, section => section.data())
  ...
  data() {
    return this.getValue()
      .then(value => {
        let section = _.pick(this, props.concat(['endTime']));
        section.value = value;
        return section;
      });
  }
...
Section.sections = require('../test-data.json').sections.map(section=>
    new Section(section)
);
