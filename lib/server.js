...
function initSequelize() {
  ...
  return sequelize.sync()
    ...
    .then(() => Program.count())
    .then(count => {
      if (nodeEnv === 'development' && count == 0) {
        log.debug('No programs exist and in development mode, creating test data');
...
function initSections() {
  ...
  return Section.findAll()
    .tap(sections => log.trace({ sections }, 'sections'))
...
function initPrograms() {
  ...
  return Program.findAll()
    .tap(programs => log.trace({ programs }, 'programs'))
