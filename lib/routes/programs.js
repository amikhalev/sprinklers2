...
router.put('/:program', validate(validations.updateProgram), (req, res) => {
  req.program.update(req.body);
  req.log.debug({data: req.program.data()}, 'Updated program data');
  res.json({
    status: 200,
    message: `Updated program ${req.program.name}`,
    data: req.program.data()
  });
});
